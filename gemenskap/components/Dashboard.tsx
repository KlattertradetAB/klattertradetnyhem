...
// Custom PWA Install Card Sub-component
const PWAInstallCard: React.FC = () => {
  ...
  useEffect(() => {
    if (updateAvailable) {
      import('../services/notificationStore').then(m => {
        m.addNotification({
          title: 'Ny uppdatering tillgänglig!',
          content: 'Vi har laddat upp nya funktioner. Klicka på den blinkande lampan för att uppdatera.',
          icon: '🔄',
          color: 'bg-orange-500',
          time: 'Just nu'
        });
  ...
  // If already installed and no update, show nothing
  if (isInstalled && !updateAvailable) return null;

  // Show Update Lamp if update is available
  if (updateAvailable) {
    return (
      <div className="col-span-1 md:col-span-2 bg-gradient-to-br from-blue-600 to-indigo-700 p-8 rounded-[2rem] shadow-2xl relative overflow-hidden animate-in fade-in duration-500 border border-white/20 mb-8">
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-4">
            <div className="relative">
              <div className="w-4 h-4 bg-orange-500 rounded-full animate-ping absolute inset-0"></div>
              <div className="w-4 h-4 bg-orange-400 rounded-full relative"></div>
            </div>
            <div>
              <h4 className="text-xl font-black text-white uppercase tracking-tighter">Uppdatering tillgänglig!</h4>
              <p className="text-white/80 text-sm font-medium">En nyare version av Horizonten-appen finns redo för dig.</p>
            </div>
          </div>
          <button
            onClick={handleUpdateClick}
            className="bg-white text-blue-600 font-black px-8 py-3 rounded-xl shadow-lg hover:scale-105 transition-all text-xs uppercase tracking-[0.2em] whitespace-nowrap"
          >
            Uppdatera nu ✨
          </button>
        </div>
  ...
  return (
    <div className="col-span-1 md:col-span-2 bg-gradient-to-br from-orange-600 via-orange-500 to-amber-500 p-8 rounded-[2rem] shadow-2xl relative overflow-hidden group border border-white/20 animate-in zoom-in duration-500 mb-8">
      {/* Festive Decorations */}
      <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-30 transition-all duration-700 blur-[2px] group-hover:blur-none">
        <Bot size={180} className="text-white rotate-12 translate-x-10 translate-y-[-20px]" />
      </div>
      ...
      <div className="relative z-10">
        <div className="flex flex-wrap items-center gap-3 mb-6">
          ...
          <div className="bg-white text-orange-600 px-4 py-1.5 rounded-full font-black text-[12px] uppercase tracking-[0.2em] shadow-lg flex items-center gap-2">
            <span>🎉</span> GRATTIS!
          </div>
          <span className="text-[10px] bg-black/20 text-white px-3 py-1.5 rounded-full font-black uppercase tracking-widest backdrop-blur-sm border border-white/10">
            PREMIUM-FÖRMÅN AKTIVERAD
          </span>
        </div>

        <div className="grid md:grid-cols-2 gap-8 items-center">
          <div>
            <h4 className="text-3xl md:text-4xl font-black text-white mb-4 leading-tight drop-shadow-sm">
              Lås upp din <br />
              exklusiva app-upplevelse
            </h4>
            <p className="text-white/90 text-sm mb-8 leading-relaxed font-medium max-w-sm">
              Som Premium-medlem har du nu tillgång till vår dedikerade app. Installera den nu för en renare upplevelse och notiser direkt till låsskärmen!
            </p>
            ...
            <div className="flex flex-wrap gap-4">
              {deferredPrompt ? (
                <button
                  ...
                  className="bg-white text-orange-600 font-black px-8 py-4 rounded-2xl shadow-xl hover:scale-105 transition-all text-sm uppercase tracking-widest flex items-center gap-3 active:scale-95"
                >
                  Installera nu <ChevronRight size={18} />
                </button>
              ) : (
                <div className="text-white/70 text-xs italic font-medium bg-black/10 px-4 py-2 rounded-lg border border-white/5">
                  Appen är redo att installeras via din webbläsares meny
                </div>
              )}
            </div>
          </div>

          <div className="bg-white/10 backdrop-blur-2xl p-6 rounded-[1.5rem] border border-white/20 shadow-inner">
            <h5 className="text-[10px] text-white/70 font-black uppercase tracking-widest mb-4">Snabbguide</h5>
            <ul className="space-y-4">
              <li className="flex items-start gap-3">
                <div className="w-5 h-5 bg-orange-400 rounded-full flex items-center justify-center shrink-0 text-[10px] font-black text-white">1</div>
                <p className="text-xs text-white leading-snug">Klicka på "Installera nu" eller webbläsarens <span className="font-black">menyknape</span>.</p>
              </li>
              <li className="flex items-start gap-3">
                <div className="w-5 h-5 bg-orange-400 rounded-full flex items-center justify-center shrink-0 text-[10px] font-black text-white">2</div>
                <p className="text-xs text-white leading-snug">Välj <span className="font-black">"Lägg till på hemskärmen"</span> eller <span className="font-black">"Installera app"</span>.</p>
              </li>
              <li className="flex items-start gap-3">
                <div className="w-5 h-5 bg-orange-400 rounded-full flex items-center justify-center shrink-0 text-[10px] font-black text-white">3</div>
                <p className="text-xs text-white leading-snug">Klart! Horizonten ligger nu bland dina andra appar.</p>
              </li>
            </ul>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {user.membership_level === 3 && typeof window !== 'undefined' && (
                <PWAInstallCard />
              )}
